...
A **second bot reviewer** (e.g. **Gemini Code Assist** — free, official, a different model family from Codex) is an *optional third surface*, added when Codex-alone proves insufficient on a given repo — not a standing requirement. The earlier "reviewer triangle" doctrine assumed a paid CodeRabbit subscription and a PR-Agent bot; neither is in the pipeline now (CodeRabbit unsubscribed; PR-Agent deprecated as descriptive-only), so the redundancy rests on PRE-REVIEW + Codex.
