...
| After commit, before PR | **`codex-review` (Codex bot)** | Codex (GitHub app) is the bot reviewer: cross-cutting correctness, hypothesis-driven bug finding, operating-context-aware. It runs *downstream* of the local PRE-REVIEW layer (Claude + `pr-review-toolkit` specialists), which already eats ~50% of findings — so the deliberate redundancy is **local PRE-REVIEW + Codex**: two independent surfaces, different optimisation. Pull substantive findings into `clawpm issues add`. **Second bot reviewer: Gemini Code Assist** (official Google GitHub app — free, real bug-finding, different model family from Codex = genuine redundancy). Wired **on-demand** (`.gemini/config.yaml` disables auto-review-on-open to avoid noise); summon `/gemini review` on non-trivial PRs as the second opinion. **Deprecated:** PR-Agent (`the-pr-agent/pr-agent`) — descriptive-only, 0 actionable findings in the 2026-05-15 smoke test. **Not in the pipeline:** CodeRabbit (paid SaaS, not subscribed). |
...
### Doctrine: layered review (local PRE-REVIEW + Codex), redundancy by surface not by count

The review redundancy comes from running **two independent surfaces**, not from stacking bot reviewers. The local PRE-REVIEW layer (Claude + `pr-review-toolkit` specialists) and the **Codex** bot have different optimisation surfaces — each catches bugs the other misses. That's the deliberate redundancy; don't collapse it to "PRE-REVIEW is enough" or "Codex is enough".

**Gemini Code Assist** (free, official, a different model family from Codex) is wired as the second bot surface, **on-demand**: `.gemini/config.yaml` turns off auto-review-on-open, so it stays quiet on trivial diffs and is summoned with `/gemini review` on non-trivial PRs. The earlier "reviewer triangle" doctrine assumed a paid CodeRabbit subscription and a PR-Agent bot; neither is in the pipeline now (CodeRabbit unsubscribed; PR-Agent deprecated as descriptive-only). Gemini's different model family makes it genuine redundancy — it catches OpenAI-blindspot bugs Codex misses.
