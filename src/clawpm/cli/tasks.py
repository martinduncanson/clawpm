...
@click.option("--scope", "-s", "scope", multiple=True, help="File glob patterns claimed by this task (can specify multiple). On Windows, a pattern containing wildcards (e.g. 'scripts/**') can be expanded by the launcher before this CLI sees it — use --scope-file if that happens.")
...
@click.option("--predict-scope", "predict_scope", multiple=True, help="Predicted file glob scope (can specify multiple). On Windows, a pattern containing wildcards (e.g. 'scripts/**') can be expanded by the launcher before this CLI sees it — use --predict-scope-file if that happens.")
...
@click.option("--scope", multiple=True, help="File glob patterns claimed by this task (can specify multiple). On Windows, a pattern containing wildcards (e.g. 'scripts/**') can be expanded by the launcher before this CLI sees it — use --scope-file if that happens.")
@click.option("--scope-file", "scope_file", default=None, type=click.Path(), help="Read scope glob patterns from file (one per line). Windows-safe: bypasses CRT argv glob-expansion. Use instead of --scope when patterns contain wildcards.")
