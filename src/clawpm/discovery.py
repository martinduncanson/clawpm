...
def discover_projects(
    ...
    A single project id can appear in multiple sibling directories — e.g. a
    worktree or a parallel clone (``foo``, ``foo-experiment``, ``foo-backup``
    all holding ``id="foo"`` in their ``.project/settings.toml``). Return one
    row per id, preferring the entry whose directory name matches the id
    (the canonical checkout) over worktree clones, and sorted-first-seen
    otherwise. Sorting before iteration makes the fallback deterministic
    across filesystems — ``Path.iterdir()`` ordering is not guaranteed.
    """
